...
import co.idearun.common.FileUtils
import co.idearun.feature.BaseActivity
    ...
    fun selectedDateIsReady(
        ...
        if (checkDateLimit(strDateToSend, field)) {
            ...
            viewModel.setErrorToField(
                field ?: Fields(),
                "${getString(co.idearun.feature.R.string.from_date)} : ${
                    baseMethod.getDateOnLocale(
                        field?.from_date ?: ""
                    ) ?: "-"
                }" +
                        "  " +
                        "${getString(co.idearun.feature.R.string.to_date)} : ${
                            baseMethod.getDateOnLocale(
                                field?.to_date ?: ""
                            ) ?: "-"
                        }"
